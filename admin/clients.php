                                ...
                                <table id="example1" class="table table-bordered table-striped ">
                                    ...
                                    <tbody>
                                        <?php while($strow = $stres->fetch_assoc()){
                                            $user = $strow['user_id'];
                                            $cardqry = "SELECT * FROM card_order WHERE user_id = '$user' ORDER BY order_id DESC LIMIT 1";
                                            $cardres = $conn->query($cardqry);
                                            if($cardres->num_rows == 0){
                                                $order_status = "<span class='text-muted'>Not Ordered</span>";
                                            }else{
                                                $cardrow = $cardres->fetch_assoc();

                                                if($cardrow['order_status'] == 0){
                                                    $order_status = "<span class='text-danger'>Pending Printing</span>";
                                                }elseif($cardrow['order_status'] == 1){
                                                    $order_status = "<span class='text-warning'>Shipping</span>";
                                                }elseif($cardrow['order_status'] == 2){
                                            ...
                                            ?>
                                        <tr>
                                            <td><?php echo $strow['fname']." ". $strow['lname'];?></td>
                                            <td><?php echo $strow['email'];?></td>
                                            <td><?php echo $order_status;?></td>
                                            <td><?php echo $strow['date_created'];?></td>
                                            <td class="text-nowrap">
                                                <a class="btn btn-sm bg-olive viewuser" href="user.php?user=<?php echo $user;?>" ><i class="fas fa-eye"></i> </a>
                                            </td>
                                        </tr>
                                        <?php } ?>
                                    </tbody>
